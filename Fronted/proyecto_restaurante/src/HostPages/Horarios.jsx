...
const Horarios = () =>{
  const columnas = ['hora', 'total_pedidos'];
  ...
  const getDatos = async () => {
    ...
    const response = await fetch(`https://cocina.web05.lol/horario?fecha_inicio=${fechas.fechainicio}&fecha_fin=${fechas.fechafinal}`, fetchOptions)
    if (response.ok){
  ...
  return (
    <div className='sizesquare'>
      ...
      <h2 style={{color: 'white', fontSize: '20px', marginLeft: '16px'}}>Horario en el que se ingresan más pedidos</h2>
