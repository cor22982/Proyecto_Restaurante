...
const Queja_Comida = () => {
  const columnas = ['comida', 'total_quejas'];
  ...
  const getDatos = async () => {
    ...
    const response = await fetch(`https://cocina.web05.lol/quejasbyfood?fecha_inicio=${fechas.fechainicio}&fecha_fin=${fechas.fechafinal}`, fetchOptions)
    if (response.ok){
  ...
  return (
    <div className='sizesquare'>
      <h1 className="titulo1">Queja Comida</h1>
      <h2 style={{color: 'white', fontSize: '20px', marginLeft: '16px'}}>Reporte de las quejas agrupadas por plato</h2>
